...
bool getFileSize(const String& path, long long& result) {
  ...
  bool retval = fs->GetFileSize(Convert(path), result64);
  result = static_cast<long long>(result64);
  return retval;
}
...
bool getFileSize(PlatformFileHandle handle, long long& result) {
  auto fs = fileSystem(); if (!fs) return false;
  int64_t result64 = static_cast<int64_t>(result);
  bool retval = fs->GetFileSize((ultralight::FileHandle)handle, result64);
  result = static_cast<long long>(result64);
  return retval;
}
...
long long seekFile(PlatformFileHandle handle, long long offset, FileSeekOrigin origin) {
  ...
  int64_t retval = fs->SeekFile(handle, static_cast<int64_t>(offset), (ultralight::FileSeekOrigin)origin);
  return static_cast<long long>(retval);
}

bool truncateFile(PlatformFileHandle handle, long long offset) {
  auto fs = fileSystem(); if (!fs) return false;
  return fs->TruncateFile(handle, static_cast<int64_t>(offset));
}
