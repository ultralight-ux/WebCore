...
void RenderBox::paintBoxDecorations(PaintInfo& paintInfo, const LayoutPoint& paintOffset)
{
    ...
    else if (isBody() && skipBodyBackground(this)) {
      bgColor = Color::transparent;
    }
    else if (backgroundIsKnownToBeObscured(paintRect.location()) && !boxShadowShouldBeAppliedToBackground(paintRect.location(), bleedAvoidance)) {
      bgColor = Color::transparent;
    }
    else {
      bgColor = style().visitedDependentColor(CSSPropertyBackgroundColor);
    }

    GraphicsContext& ctx = paintInfo.context();
    ...
    if (paintNinePieceImage(ctx, paintRect, style(), style().borderImage()))
      return;

    BorderEdge edges[4];
    BorderEdge::getBorderEdgeInfo(edges, style(), document().deviceScaleFactor(), true, true);
    RoundedRect outerBorder = style().getRoundedBorderFor(paintRect, true, true);
    RoundedRect innerBorder = style().getRoundedInnerBorderFor(borderInnerRectAdjustedForBleedAvoidance(ctx, paintRect, bleedAvoidance), true, true);
    ...
    PlatformGraphicsContext* platformCtx = ctx.platformContext();
    platformCtx->DrawBoxDecorations(paintRect, pixelSnappedOuterBorder, pixelSnappedInnerBorder, edges, bgColor);
#else
    ...
    bleedAvoidance = BackgroundBleedNone;
#endif
